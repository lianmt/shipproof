...
export async function runBenchmark(outputDir) {
    ...
    for (const benchmarkCase of cases) {
        ...
        try {
            ...
            await writeFile(path.join(cwd, "shipproof.yml"), YAML.stringify({
                version: 1,
            ...
            if (benchmarkCase.mutate)
                await benchmarkCase.mutate(cwd);
            if (benchmarkCase.beforeVerify)
                cleanupExternal = await benchmarkCase.beforeVerify();
            let run = await verify({ cwd, taskPath: "task.md", requireLock: true });
    ...
    const result = {
        ...
        falseVerified: results.filter((item) => item.actual === "VERIFIED" && item.expected !== "VERIFIED").length,
        cases: results,
    };
...
function renderBenchmark(result) {
    return [
        ...
        "|---|---|---|---|",
        ...result.cases.map((item) => `| ${item.id} | ${item.expected} | ${item.actual} | ${item.passed ? "PASS" : "FAIL"} |`),
        "",
    ].join("\n");
}
...
function commandCheck(run, extra = {}) {
    return { id: "check", type: "command", run, required: true, ...extra };
}
function commandCase(id, expected, run) {
    return { id, expected, check: commandCheck(run) };
}
function fileCase(id, expected, extra) {
    return { id, expected, check: { id: "check", type: "file", required: true, ...extra } };
}
function httpCase(id, expected, port, actualStatus, actualBody, expectStatus, contains) {
...
async function startServer(port, status, body) {
