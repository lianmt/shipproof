...
export async function writeRunArtifacts(run, evidenceRoot) {
...
export function renderMarkdown(run) {
    ...
    lines.push("## Checks", "", "| Check | Type | Required | Result | Duration | Summary |", "|---|---|---:|---|---:|---|");
    for (const check of run.checks) {
        lines.push(`| ${escapeCell(check.name)} | ${check.type} | ${check.required ? "yes" : "no"} | ${check.status} | ${check.durationMs} ms | ${escapeCell(check.summary)} |`);
    }
    lines.push("", "## Integrity", "", `- Configuration SHA-256: \`${run.integrityAfter.configHash}\``, `- Protected set SHA-256: \`${run.integrityAfter.protectedHash}\``, `- Workspace SHA-256: \`${run.integrityAfter.workspaceHash}\``, `- Task SHA-256: ${run.integrityAfter.taskHash ? `\`${run.integrityAfter.taskHash}\`` : "not supplied"}`, "", "> A VERIFIED result is valid only for the recorded workspace and acceptance hashes.", "");
    return lines.join("\n");
}
export async function readLatestRun(options) {
    if (options.runPath) {
        return JSON.parse(await readFile(path.resolve(options.cwd, options.runPath), "utf8"));
    }
...
export async function refreshRunFreshness(run) {
...
export async function listRunIds(evidenceRoot) {
    try {
