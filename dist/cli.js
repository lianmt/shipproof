...
import { handleCodexHook, installCodexHooks } from "./codex-hook.js";
...
program
    ...
    if (options.json)
        ...
        printRunSummary(run.status, run.reportPath, run.reasons);
...
program
    ...
    try {
        ...
        process.stdout.write(`${JSON.stringify({ continue: false, stopReason: `ShipProof hook error: ${message(error)}` })}\n`);
        process.exitCode = 1;
    }
});
program
...
async function inferConfig(cwd, configPath) {
...
function printRunSummary(status, reportPath, reasons) {
