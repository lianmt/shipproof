...
export async function createLock(options: {
  ...
  await writeFile(
    path.join(options.cwd, LOCK_FILE),
...
export async function readLock(cwd: string): Promise<VerificationLock | null> {
  try {
    const raw = await readFile(path.join(cwd, LOCK_FILE), "utf8");
    return JSON.parse(raw) as VerificationLock;
  } catch (error) {
    if (
      error &&
      typeof error === "object" &&
...
export function validateIntegrity(
  ...
  if (!lock && requireLock) reasons.push("verification lock is missing; run shipproof lock before implementation");
  if (lock) {
    if (lock.configHash !== before.configHash) reasons.push("configuration changed after the lock was created");
    if (lock.protectedHash !== before.protectedHash) reasons.push("protected acceptance files changed after the lock was created");
    if (lock.taskHash !== before.taskHash) reasons.push("task specification changed after the lock was created");
  }
  if (before.configHash !== after.configHash) reasons.push("configuration changed during verification");
  if (before.protectedHash !== after.protectedHash) reasons.push("protected files changed during verification");
  if (before.taskHash !== after.taskHash) reasons.push("task specification changed during verification");
  return { valid: reasons.length === 0, reasons };
}

function withConfig(config: ShipProofConfig, configPath: string): string[] {
