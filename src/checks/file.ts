...
import type { CheckResult, FileCheck } from "../types.js";
...
export async function runFileCheck(
  ...
  try {
    ...
    return createResult(
      ...
      errors.length === 0 ? "file check passed" : errors.join("; "),
      { path: check.path, size: fileStat.size, sha256: await hashFile(absolutePath) },
    );
  } catch (error) {
    ...
    return createResult(
      check,
      startedAt,
      isNotFound(error) ? "FAILED" : "BLOCKED",
      isNotFound(error) ? "required file is missing" : `cannot inspect file: ${toMessage(error)}`,
      { path: check.path },
    );
  }
}
